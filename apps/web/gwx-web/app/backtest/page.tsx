  ...
  async function onCsv(e: ChangeEvent<HTMLInputElement>){
    ...
    try{
      ...
      const qs = new URLSearchParams({
        symbol,
        risk_percent: String(riskPct),
        account_equity: String(equity),
        atr_mult_sl: String(atrMult),
        tp_r_multiple: String(tpR),
        min_warmup: String(60),
      }).toString();
      const r = await fetch(`${API_URL}/backtest/csv?${qs}`, {
        method: "POST",
    ...
    <div className="container-narrow space-y-4">
      ...
      <div className="card p-4 space-y-3">
        <div className="flex flex-wrap gap-2 items-center">
          <input className="border border-neutral-300 rounded-xl px-3 py-2 w-36" value={symbol} onChange={e=>setSymbol(e.target.value)} placeholder="Symbol"/>
          ...
          <input className="border border-neutral-300 rounded-xl px-3 py-2 w-44" type="number" step="100" value={equity} onChange={e=>setEquity(parseFloat(e.target.value))}/>
          <span className="text-sm">Account size (£)</span>
          <input className="border border-neutral-300 rounded-xl px-3 py-2 w-40" type="number" step="0.1" value={atrMult} onChange={e=>setAtrMult(parseFloat(e.target.value))}/>
          <span className="text-sm">Stop distance (ATR×)</span>
          <input className="border border-neutral-300 rounded-xl px-3 py-2 w-40" type="number" step="0.1" value={tpR} onChange={e=>setTpR(parseFloat(e.target.value))}/>
        ...
        <p className="text-xs text-neutral-500">
          CSV columns: <code>ts,open,high,low,close,volume</code>. Tip: export ~100 bars so indicators have time to warm up.
        </p>
      </div>
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
            <div className="card p-4">
              ...
              <div className="text-2xl font-semibold">{res.count}</div>
            </div>
            <div className="card p-4">
              <div className="text-xs text-neutral-500">Win rate</div>
              <div className="text-2xl font-semibold">{(res.winrate ?? 0).toFixed(1)}%</div>
            </div>
            <div className="card p-4">
              <div className="text-xs text-neutral-500">Net P/L</div>
              <div className="text-2xl font-semibold">£{(res.net_pnl ?? 0).toFixed(2)}</div>
            </div>
            <div className="card p-4">
              <div className="text-xs text-neutral-500">Equity (end)</div>
              <div className="text-2xl font-semibold">£{(res.equity_end ?? 0).toFixed(2)}</div>
            </div>
          </div>
